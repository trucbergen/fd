...
use_db <- function(conn, db){
  a <- DBI::dbGetQuery(conn, glue::glue({"SHOW DATABASES LIKE '{db}';"}))
  if (nrow(a) == 0) {
    a <- DBI::dbExecute(conn, glue::glue({"CREATE DATABASE {db};"}))
  }
  a <- DBI::dbExecute(conn, glue::glue({"USE {db};"}))
}
...
get_field_types <- function(conn, dt){
  field_types <- vapply(dt, DBI::dbDataType, dbObj = conn,
...
drop_all_rows <- function(conn, table){
  a <- DBI::dbExecute(conn, glue::glue({"DELETE FROM {table};"}))
}
